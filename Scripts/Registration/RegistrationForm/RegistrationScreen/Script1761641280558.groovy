...
CustomKeywords.'registrationform.student_registrationform.verifyURL'('https://demoqa.com/automation-practice-form')
...
for (TestObject element : elements) {
	if (WebUI.verifyElementVisible(element, FailureHandling.OPTIONAL)) {
		KeywordUtil.markPassed("Element ${element.getObjectId()} terlihat")
	} else {
		KeywordUtil.markFailed("Element ${element.getObjectId()} tidak terlihat")
	}
